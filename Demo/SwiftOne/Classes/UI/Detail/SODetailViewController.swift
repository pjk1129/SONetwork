//
//  SODetailViewController.swift
//  SwiftOne
//
//  Created by JK.PENG on 2017/3/28.
...
class SODetailViewController: SOBaseViewController {
    ...
    override func viewDidLoad() {
        ...
        backBtnShown = true
        navTitle = NSString(string: goodsItem.name)
        setupUIConstraints()
        reloadWebView()
    }
    
    private func reloadWebView() {
        let dic = parametersFromQuery(goodsItem.url)
        
        let url = NSURL(string: dic.object(forKey: "url") as! String) as! URL
...
extension SODetailViewController: WKNavigationDelegate {
    // 页面开始加载时调用
